...
const _fest = {
    ...
    parse:function(subtree,exportsOnly){
        var i,cst="",apped=false;
        if(subtree.exports.length>0){
            cst+="const {"
            for(i=0;i<subtree.exports.length-1;i++){
                //Ensures we export only if not exported
                if(!apps.includes(subtree.exports[i])){
                    apps.push(subtree.exports[i]);
                    apped=true;
                    cst+=subtree.exports[i]+","
                }
            };
            if(!apps.includes(subtree.exports[subtree.exports.length-1])){
                apps.push(subtree.exports[subtree.exports.length-1]);
                cst+=`${subtree.exports[subtree.exports.length-1]}}=`
            }else{
                if(apped){
                    cst+="}=";
                }else{
       ...
       code = `${code.replace(/<fest>/gs,"`").replace(/<\/fest>/gs,"`")};`;
        fs.writeFileSync(subtree.path+".js",`${code.replace(/[\s]export[\s]default[\s]/,'\n;module.exports= ')}\n`);
    ...
    compile:function(config){
        ...
        for(i in config.scripts){
            if(config.scripts[i].compile){
                codes.push(config.scripts[i].path);
            }
        }
            ...
            for(i in config.scripts){
                if(config.scripts[i].compile){
                    ...
                    if(config.scripts[i].exports){
                        ...
                        fs.appendFileSync(config.compileTo,babel.transformSync(`${exporters}(function(){${fs.readFileSync(config.scripts[i].path+".js","utf8").replace(/\/\/<(.*?)\/\/>/gs,"").replace(/;module\.exports=[\s]/,';return ')}})();\n`,{
                            generatorOpts:{
                                comments:false,
                                minified:config.production?true:config["white-space"]?false:true
                            },
